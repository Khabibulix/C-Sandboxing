...
int main(){
    ...
    long file_size = get_file_size("dummy_obo.bin");
    if (file_size <= 0) return ERR_INVALID_FILE;
    if (file_size < sizeof(struct header)) return ERR_HEADER_TRUNC;
    if (file_size > sizeof(struct header) + MAX_PAYLOAD_SIZE) return ERR_INVALID_FILE;

    file = fopen("dummy_obo.bin", "rb");
    if (!file) return ERR_FILE_OPEN;


    if (fread(&h, sizeof(h), 1, file) != 1) return ERR_HEADER_INVALID;
    if (memcmp(h.magic, "BDMP", 4) != 0) return ERR_HEADER_INVALID;
    if (h.version != 1) return ERR_HEADER_INVALID;
    ...
    if (fread(payload, 1, h.payload_size + 1, file) < h.payload_size){
        free(payload);
